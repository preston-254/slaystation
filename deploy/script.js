    ...
    productsToShow.forEach(product => {
        ...
        productCard.innerHTML = `
            ...
            <div class="social-share">
                <button class="share-btn share-instagram" onclick="shareProduct('${product.name}', '${product.image}', 'instagram')" title="Share on Instagram">📷</button>
                <button class="share-btn share-whatsapp" onclick="shareProduct('${product.name}', '${product.image}', 'whatsapp')" title="Share on WhatsApp">💬</button>
                <button class="share-btn share-facebook" onclick="shareProduct('${product.name}', '${product.image}', 'facebook')" title="Share on Facebook">📘</button>
            </div>
...
const videoData = [
    {
        title: "Bag Styling Tips",
        description: "Learn how to style your couch bags for any occasion!",
        poster: "images/bags/IMG_1328.jpg",
        sources: [
            { src: "videos/bag-showcase-1.mp4", type: "video/mp4" },
            { src: "videos/bag-showcase-1.webm", type: "video/webm" }
        ]
    },
    {
        title: "Unboxing Experience",
        description: "See what it's like to receive your Slay Station bag!",
        poster: "images/bags/IMG_1331.jpg",
        sources: [
            { src: "videos/bag-showcase-2.mp4", type: "video/mp4" },
            { src: "videos/bag-showcase-2.webm", type: "video/webm" }
        ]
    },
    {
        title: "Customer Reviews",
        description: "Hear from our happy customers!",
        poster: "images/bags/IMG_1335.jpg",
        sources: [
            { src: "videos/customer-reviews.mp4", type: "video/mp4" },
            { src: "videos/customer-reviews.webm", type: "video/webm" }
        ]
    }
];

// Check if video file exists (without causing 404 errors)
...
// Render Videos (only if they exist) - with proper error handling
async function renderVideos() {
    ...
    // Check which videos exist before trying to load them
    for (const video of videoData) {
        let videoExists = false;
        
        // Check if any source exists
        for (const source of video.sources) {
            const exists = await checkVideoExists(source.src);
            if (exists) {
                videoExists = true;
                break;
            }
        ...
        if (!videoExists) {
            continue; // Skip this video - it doesn't exist
        }
        ...
        videoElement.className = 'product-video';
        videoElement.preload = 'metadata';
        
        // Add error handling - hide card if video fails
        videoElement.addEventListener('error', function() {
            videoCard.style.display = 'none';
            videosLoaded--;
            if (videosLoaded === 0 && videoPlaceholder) {
    ...
    // If no videos loaded, show placeholder
    setTimeout(() => {
        if (videosLoaded === 0 && videoPlaceholder) {
            videoPlaceholder.style.display = 'block';
            videoGrid.style.display = 'none';
...
async function checkHeroVideo() {
    ...
    const video = heroVideoContainer.querySelector('.hero-video');
    if (!video) return;
    ...
    if (!videoExists) {
        return; // Don't show video container - no 404 errors
    }
    
    // Video exists, set up loading
    ...
    video.addEventListener('loadedmetadata', function() {
        ...
        video.play().catch(() => {
            // Autoplay failed, but video exists - that's okay
        });
    }, { once: true });
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 2rem; padding: 2rem;">
                ...
                <div class="product-details-large">
                    ...
                    <div class="social-share">
                        ...
                        <button class="share-btn share-whatsapp" onclick="shareProduct('${product.name}', '${product.image}', 'whatsapp')" title="Share on WhatsApp">💬</button>
                        <button class="share-btn share-facebook" onclick="shareProduct('${product.name}', '${product.image}', 'facebook')" title="Share on Facebook">📘</button>
                        <button class="share-btn share-twitter" onclick="shareProduct('${product.name}', '${product.image}', 'twitter')" title="Share on Twitter">🐦</button>
                    </div>
                </div>
            </div>
        </div>
